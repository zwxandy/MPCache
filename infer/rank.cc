...
std::vector<spu::Value> TopkApply(SPUContext *ctx, const spu::Value &input,
  ...
  std::vector<std::unique_ptr<spu::SPUContext>> sub_ctxs;
  for (int64_t i = 0; i < N; i++) {
    sub_ctxs.push_back(ctx->fork());
  }

  std::vector<std::future<std::vector<spu::Value>>> futures;  // 存储每个异步任务的结果
  for (int64_t i = 0; i < N; i++) {
    auto async_res = std::async(
      [&](int64_t index) {
        auto input_i = hal::reshape(sub_ctxs[index].get(), hal::slice(sub_ctxs[index].get(), reshaped, {index, 0}, {index + 1, W}), {W});
        return apply_fn(sub_ctxs[index].get(), input_i);
      }, i);
  ...
  // 等待所有异步操作完成
  for (int64_t i = 0; i < N; i++) {
    topk1d.push_back(futures[i].get());  // 获取每个异步任务的结果
  }
...
std::vector<spu::Value> TopK(SPUContext *ctx, const spu::Value &input,
  ...
  auto topk_fn = [&](SPUContext* forked_ctx, const spu::Value &input) {
    return hal::topk_1d(forked_ctx, input, scalar_cmp_fn, config);
  };
