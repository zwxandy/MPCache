...
def get_pred(rank, world_size, data, max_length, max_gen, prompt_format, dataset, device, model_name, model2path, out_path):
    ...
    for json_obj in tqdm(data):
        ...
        with open(out_path, "a", encoding="utf-8") as f:
            ...
            f.write('\n')
        # print(f'Current avg context length: {sum(context_length_list) / len(context_length_list)}')
    # print(f'Overall avg context length: {sum(context_length_list) / len(context_length_list)}')
...
if __name__ == '__main__':
    ...
    if not os.path.exists("pred_mine"):
        os.makedirs("pred_mine")
